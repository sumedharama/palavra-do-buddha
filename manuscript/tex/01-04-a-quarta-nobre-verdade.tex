...
\begin{quote}
  ...
  condição imediata para entrar nos quatro Estágios Sagrados (ver p. 64 f. FIXME
  pageref) e alcançar o \emph{Nibbāna}.
...
\begin{quote}
  ...
  Para as dez ``prisões'' (\emph{sa\.{m}yojana}), ver na página no 61 FIXME pageref
\end{quote}
...
Quer os ``Seres Perfeitos'' (\emph{Buddhas}) apareçam no mundo ou não, ainda
...
\begin{quote}
  O termo ``saṅkhārā'' (formações) abarca aqui todas as coisas que são
...
\begin{quote}
  O diagrama seguinte dá um panorama de como as doze ligações da fórmula se
  estendem sobre três existências consecutivas - passado, presente e futuro.
\end{quote}
...
\begin{verse}
  ...
  \quoteRef{Vism XVII, 291}
\end{verse}

\begin{quote}
  Para mais pormenores, ver Fund. III e \emph{B. Dict.}
\end{quote}
...
\begin{quote}
  ...
  absorções (\emph{jhāna}; ver pp. 109-113 f. FIXME pageref); para o
...
\begin{enumerate}
  \setcounter{enumi}{4}
  \item Poderá desfrutar os diferentes poderes mágicos (\emph{iddhi-vidhā}).
...
\begin{quote}
  ...
  Existem quatro \emph{iddhipāda}, ou ``bases para se obterem poderes mágicos'',
...
A consciência da respiração, na inspiração e na expiração (\emph{ānāpānasati}),
...
\begin{quote}
  ...
  Ver \emph{B. Dict.}: \emph{kasiṇa, nimitta, samādhi}.
\end{quote}
...
\begin{quote}
  ...
  ``Concentração aproximada'' (\emph{upacarā-samādhi}): através do relembrar do
  ...
  Todos os objectos e métodos de concentração são tratados no \emph{Vism.
    IIIXIII}; ver também \emph{Fund. IV}.
\end{quote}
